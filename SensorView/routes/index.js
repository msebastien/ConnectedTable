...
router.get('/connectedTableView',function(req,res){
    connectToInfluxDB('connected_table');
...
router.post('/table', function(req,res) {
    ...
    busboy.on('finish', function () {
        sendDataToInfluxDB("connected_table", value);
...
router.post('/airquality', function(req,res) {
    ...
    busboy.on('finish', function () {
        sendDataToInfluxDB("qarpediem", value);
        res.writeHead(303, {Connection: 'close', Location: '/'});
        res.end();
    });
    req.pipe(busboy);
...
/**
*   CONNECTED TABLE DATA (JSON)
**/
router.post('/connectedTableJSON', function (req,res){
    var data = req.body;
    for(var i=0; i<data.length; i++){
        sendDataToInfluxDB("connected_table", data[i]);
    }
    res.status(200).end();
});
