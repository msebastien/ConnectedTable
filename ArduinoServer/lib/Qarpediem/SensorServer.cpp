...
SensorServer::SensorServer(HardwareSerial *Serial)
    : serial_port(Serial), last_poll_average(0) {}
//, last_ping_dust(0)
...
void SensorServer::init(void)
{
    ...
    pinMode(Qarpediem::DATA_SENT_SIGNAL_PIN, OUTPUT);
...
void SensorServer::on_sensor(String sensor_name)
{
    ...
    {
        //float sound = sound_sensor.get_average_level() / 1024.0 * 5000.0;
        float sound = (float)(sound_sensor.get_average_level() * 50.0 / 1023.0);
        Serial.print("Sound:");
